"""hf_cluster_optimizer.models.lightgbm -- LightGBM gradient-boosting trainer."""
...
class LightGBMTrainer(Trainer):
    """LightGBM model with classification or regression."""
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> "LightGBMTrainer":
        return cls(config)
    ...
    def train_step(self, batch: Any) -> TrainerStepResult:
        ...
        self.model = lgb.train(
            self.params, dset, num_boost_round=self.num_boost_round,
        )
        s = self._score(Xb, yb)
        self._train_score = s
